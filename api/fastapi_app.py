...
GET  /health  - JSON status + whether the model artifact is loaded
...
def predict(features: CustomerFeatures) -> PredictionResponse:
    ...
    Returns a 503 if the model artifact is not available (i.e. you
    have not run ``python run_training.py`` yet) and a 422 for any
    schema validation issue (handled automatically by FastAPI).
    """
